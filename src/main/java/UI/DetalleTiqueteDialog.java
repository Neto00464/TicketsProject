...
public class DetalleTiqueteDialog extends JDialog implements ChatClient.ChatMessageListener {
    ...
    try {
        List<MensajeDAO.Mensaje> mensajesAnteriores = mensajeDAO.obtenerMensajesPorTiquete(tiquete.getId());
        
        if (!mensajesAnteriores.isEmpty()) {
            chatArea.append("--- Mensajes anteriores ---\n\n");
            
            java.time.format.DateTimeFormatter formatter = 
                java.time.format.DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm");
            
            for (MensajeDAO.Mensaje msg : mensajesAnteriores) {
                String fechaStr = msg.getFecha() != null ? 
                                 msg.getFecha().format(formatter) : "";
                chatArea.append("[" + fechaStr + "] " + msg.getUsuarioNombre() + 
                              ": " + msg.getContenido() + "\n");
            }
            
            chatArea.append("\n--- Fin de mensajes anteriores ---\n\n");
        }
        
    } catch (SQLException ex) {
        chatArea.append("(No se pudieron cargar mensajes anteriores)\n\n");
        System.err.println("Error cargando mensajes: " + ex.getMessage());
    }
}
    ...
    try {
        boolean guardado = mensajeDAO.guardarMensaje(
            tiquete.getId(), 
            usuario.getId(), 
            mensaje
        );
        ...
        if (!guardado) {
            System.err.println("Advertencia: El mensaje no se guardó en la BD");
        }
    } catch (SQLException ex) {
        System.err.println("Error al guardar mensaje: " + ex.getMessage());
        ex.printStackTrace();
    }
